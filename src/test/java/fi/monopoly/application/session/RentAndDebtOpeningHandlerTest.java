...
class RentAndDebtOpeningHandlerTest {
    ...
    void immediatePaymentDoesNotOpenDebt() {
        ...
        RentAndDebtOpeningHandler handler = new RentAndDebtOpeningHandler(debtRef::set, paymentGateway);

        handler.handle(new PaymentRequest(debtor, BankTarget.INSTANCE, 100, "Rent"), () -> callbackTriggered.set(true));
    ...
    void insufficientCashOpensDebtWithoutBankruptcyRisk() {
        ...
        handler.handle(new PaymentRequest(debtor, BankTarget.INSTANCE, 100, "Rent"), () -> {
        });
